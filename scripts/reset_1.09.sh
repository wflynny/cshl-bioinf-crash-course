#!/usr/bin/env bash
#
# reset_1.09.sh — restore $COURSE/work/1.09 to a clean starting state.
#
# For the TA: if a student wedges module 1.09 (Putting It All Together), this
# wipes their 1.09 working dir and rebuilds the clean inputs:
#   $COURSE/work/1.09/fastqs/     symlinks to each tiny FASTQ
#   $COURSE/work/1.09/tiny_ref    symlink to the tiny reference
#
# Usage:
#   bash reset_1.09.sh            # CSHL HPC (default)
#   bash reset_1.09.sh --local    # local JAX testing on Elion (same ROOT as setup)
#
# WARNING: this deletes everything under $COURSE/work/1.09 (including any
...
: "${COURSE:?COURSE is empty — refusing to rm -rf}"   # guard against rm -rf /work/1.09 on an unset path
...
echo "1.09 reset (profile: ${PROFILE})."
